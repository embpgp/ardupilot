...
void drive(uint16_t hz_speed) {
    ...
    while (1) {
        for (uint8_t i = 0; i < 14; i++) {
            ...
            hal.console->printf("the %d pwm value is %d and freq is %u\n",i+1,pwm,hal.rcout->get_freq(i));
            //pwm += delta;
          //  if (delta > 0 && pwm >= 2000) {
           //     delta = -1;
                //hal.console->printf("reversing and pwm value is %d\n",pwm);
          //  } else if (delta < 0 && pwm <= 1000) {
          //      delta = 1;
                //hal.console->printf("normalizing and pwm value is %d\n",pwm);
            //}
        }
